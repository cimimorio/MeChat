    ...
    @IBAction func loginBtnClicked(_ sender: Any) {
        IMChatManager.share().mLogin(withAccount: accountLabel.text, andToken: passwordLabel.text) { [weak self](error) in
            if error != nil{
                ...
                let chaVC:ChatViewController = mainSB.instantiateViewController(withIdentifier: "ChatViewController") as! ChatViewController
                self?.present(chaVC, animated: true, completion: { 
